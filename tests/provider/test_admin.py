...
class TestApplicationAdmin:
    ...
    def test_change_view(self, admin_client):
        group = ApplicationPermissionGroupFactory.create(
            application=self.application)
        ApplicationUserFactory.create(
            application=self.application, groups=[group])
        url = reverse(
            'admin:{}_{}_change'.format(
                self.application._meta.app_label,
                self.application._meta.model_name),
...
class TestApplicationPermissionGroupAdmin:
    ...
    def test_change_view(self, admin_client):
        ...
        url = reverse(
            'admin:{}_{}_change'.format(
                self.perms_group._meta.app_label,
                self.perms_group._meta.model_name),
...
class TestApplicationUserAdmin:
    ...
    def test_add_view(self, admin_client):
        ...
        fields = [name for name in form.fields.keys()]
        assert fields == ([
            'user', 'application', 'is_superuser', 'is_staff', 'is_active'])
